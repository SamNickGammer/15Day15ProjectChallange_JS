...
const inputValueRange = document.querySelector('.inputValueRange');

slider.querySelector('input').addEventListener('input', (event) => {
  applyFill(event.target);
  sliderValue.setAttribute('data-length', event.target.value);
  inputValueRange.value = event.target.value;
});

inputValueRange.addEventListener('input', (event) => {
  if (event.target.value >= 4 && event.target.value <= 32) {
    sliderValue.setAttribute('data-length', event.target.value);
    slider.querySelector('input').value = event.target.value;
    applyFill(slider.querySelector('input'));
  }
  if (event.target.value < 4) {
    sliderValue.setAttribute('data-length', 4);
    slider.querySelector('input').value = 4;
    applyFill(slider.querySelector('input'));
  }
  if (event.target.value > 32) {
    sliderValue.setAttribute('data-length', 32);
    slider.querySelector('input').value = 32;
    applyFill(slider.querySelector('input'));
  }
});
